...
def update_question_view(request,pk):
    ...
    if request.method=='POST':
        ...
        if questionForm.is_valid():
            course = QMODEL.Course.objects.get(id=request.POST.get('courseID'))
            questionForm.instance.course = course
            questionForm.save()
            return HttpResponseRedirect('/teacher/teacher-view-question')
        else:
            print("form is invalid")
...
@login_required(login_url='teacherlogin')
@user_passes_test(is_teacher)
def detail_question_view(request,pk):
    question = get_object_or_404(QMODEL.Question,id=pk)
    context = {
        'question':question
    }

    return render(request, 'teacher/detail_question.html',context)

@login_required(login_url='teacherlogin')
def teacher_combined_view(request):
    students = SMODEL.Student.objects.all()
    courses = QMODEL.Course.objects.all()

    results_data = []

    # Fetch results for each student and course
    for student in students:
        for course in courses:
            results = QMODEL.Result.objects.filter(student=student, exam=course)
            if results.exists():
                # If results exist for the combination, add them to the data
                results_data.append({
                    'student': student,
                    'course': course,
                    'results': results,
    ...
    return render(request, 'teacher/teacher_combined_view.html', context)
